...
$.widget( "ui.geo_autocomplete", {
	...
	options: {
		...
		geocoder_types: 'street_address,locality,political,sublocality,neighborhood,country,airport,intersection,park', // array of acceptable location types, see http://code.google.com/apis/maps/documentation/javascript/services.html#GeocodingAddressTypes
		...
		source: function(_request, _response) {
			if (_request.term in this.options._cache) {
				...
				var self = this, limit = this.options.limit;
				var _address = _request.term + (this.options.geocoder_region ? ', ' + this.options.geocoder_region : '');
				this.options._geocoder.geocode({'address': _address, 'region': this.options.region, 'language': this.options.language}, function(_results, _status) {
					var _parsed = [];
